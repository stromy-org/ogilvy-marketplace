# Ogilvy Dominicana Plugin Marketplace

Public marketplace for Ogilvy Dominicana Claude Code plugins.
...
2. Click the `+` to add a marketplace → enter `stromy-org/ogilvy-marketplace`
3. Click **Ogilvy Dominicana** → Install

### Option B: From the CLI

```bash
# Add marketplace (one-time)
claude plugin marketplace add stromy-org/ogilvy-marketplace

# Install plugin
claude plugin install ogilvy-plugin@ogilvy-marketplace
```

### Post-install: dependencies (one-time)

```bash
cd ~/.claude/plugins/cache/ogilvy-marketplace/ogilvy-plugin/0.1.0
...
| Skill | Command |
|-------|---------|
| _example_ | `/ogilvy-plugin:example` |

## Updating

```bash
claude plugin update ogilvy-plugin@ogilvy-marketplace
```
...
| "Plugin not found in marketplace" | Run `claude plugin marketplace add stromy-org/ogilvy-marketplace` first |

## Architecture

- **Marketplace** (this repo): public — hosts `marketplace.json` only
- **Plugin** (`ogilvy-plugin`): public — contains skills, brand data, company info
- **Source format**: use `"source": "url"` with an explicit HTTPS clone URL ending in `.git`, for example `https://github.com/stromy-org/ogilvy-plugin.git`
